...
class TranslatorThread(QThread):
    ...
    def _build_system_prompt(self, lang_english):
        base = (
            ...
            f"Input: JSON (keys = identifiers, NEVER change). Values = English text.\n"
            f"Output: JSON with same keys, translated values. Raw JSON only, no markdown.\n"
            f"Keep ALL ⟨⟩ tokens and numeric values EXACTLY as-is.\n\n"
            f"=== TEXT TYPE HANDLING ===\n"
            f"Identify EACH value's type and apply the right strategy:\n\n"
            f"1. Noun phrases (item/block/enchant/skill names, UI labels):\n"
            f"   Concise noun phrase. DO NOT make into a sentence.\n"
            f"   'Molten Pickaxe'→'溶岩のツルハシ', 'Nether Star'→'ネザースター'\n\n"
            f"2. Stat lines (short lines with numbers, %, +/-, keywords):\n"
            f"   Keep as short fragment. DO NOT expand into a full sentence.\n"
            f"   '+10% Crit Chance'→'クリティカル率+10%' (NOT 'クリティカル率が10%上昇する')\n"
            f"   '3-block radius'→'半径3ブロック', '150%'→'150%'\n\n"
            f"3. Descriptions (sentences of explanatory text, multi-clause descriptions):\n"
            f"   Restructure into natural Japanese. REASON before RESULT.\n"
            f"   'X because Y'→'YなのでX'. Translate meaning, not words.\n"
            ...
            f"   VERB CHAINING — NEVER repeat the same connector:\n"
            f"   When a sentence has 3+ clauses joined by commas, vary the connectors:\n"
            f"   連用形(〜し/〜て), とともに, および/及び, ながら, つつ, 等.\n"
            f"   BAD: 「アイテムを汚染し、変更不可にし、アフィックスを追加する」(し×3)\n"
            f"   GOOD: 「アイテムを破損させ、変更不可能にするとともに、アフィックスを追加する」\n\n"
            f"4. Multi-line values:\n"
            f"   If each line is an independent stat/effect → translate each line INDEPENDENTLY.\n"
            f"   If one coherent paragraph → treat as single text.\n"
            f"   When in doubt, preserve line-by-line structure. DO NOT merge lines.\n\n"
            f"=== STYLE ===\n"
            f"常体 (だ・である調). NOT 敬体 (です・ます調).\n"
            f"'Increases attack power'→'攻撃力が上がる' NOT '攻撃力が上がります'\n\n"
            f"Japanese conciseness rules:\n"
            f"- '〜することができる'→'〜できる'\n"
            f"- '〜を行う'→'〜する'\n"
            f"- '〜についての'→'〜の'\n"
            f"- NEVER '〜としてのX': 'Physical as Extra Damage'→'物理追加ダメージ'\n"
            f"- English 1 line = Japanese 1 line. DO NOT split or merge lines.\n"
            f"- Keep source length ratio. DO NOT inflate short phrases into sentences.\n\n"
            f"=== FORMATTING TOKENS ===\n"
            f"⟨...⟩ = Minecraft formatting codes (color, bold, reset). NOT text content.\n"
            f"Translate surrounding text naturally, then position ⟨...⟩ tokens where they belong.\n\n"
            f"=== TERMINOLOGY ===\n"
            f"Title Case (each word capitalized) → proper noun → カタカナ:\n"
            f"  'Iron Golem'→'アイアンゴーレム', 'Nether Portal'→'ネザーポータル'\n"
            f"lowercase → general term → 日本語化:\n"
            f"  'melee weapon'→'近接武器', 'fire resistance'→'火炎耐性'\n"
            f"Hyphenated compounds: translate all elements. 'Mini-Boss'→'ミニボス', 'Elite Mob'→'エリートモブ'\n"
            f"Creature compounds: general→日本語 ('lava squid'→'溶岩イカ'), proper→カタカナ\n"
            f"Verbs→日本語: 'craft'→'作成する', 'smelt'→'精錬する' (established: 'spawn'→'スポーンする')\n"
            f"Structure: 'Act I'→'第1章', 'Chapter 1'→'第1章', 'Wave 1'→'第1波'\n"
            f"Constants: 'mod/mods'→'MOD', 'gear/equipment'→'装備', 'stats'→'ステータス'\n"
            f"Glossary terms: use EXACTLY as specified. Non-matching translations will be rejected.\n\n"
            f"=== STAT CONVERSION ===\n"
            f"'X as Extra Y' = XダメージをYとして追加する、という意味。動詞句で自然に:\n"
            f"'Physical as Extra Damage'→'物理ダメージを追加'\n"
            f"'Physical as Extra Chaos Damage'→'物理ダメージを混沌ダメージとして追加'\n"
            f"'Lightning as Extra Physical Damage'→'雷ダメージを物理ダメージとして追加'\n"
            f"Rule: [Xダメージ]を[Yダメージとして]追加。X=Yの場合は「Xダメージを追加」で省略可。\n"
            f"NEVER '〜としてのX'(名詞化禁止), NEVER arrow(→).\n"
        )
        ...
        if self.source_type == "ftbquest":
            base += (
                ...
                "- quest.title: 短い名詞句。例: 「鉄の装備」「石炭探し」\n"
                "- quest.subtitle: クエストの雰囲気を示す1文。\n"
                "- quest.description: 常体で統一。叙述は「〜だ。〜である。」、指示は「〜せよ。」「〜すること。」。\n"
                "  同一description内で文体を混在させない。\n"
        ...
        if self.glossary:
            ...
            if relevant_terms:
                ...
                system_content += (
                    ...
                    f"Non-matching translations will be rejected. You MUST comply.\n"
                    f"{glossary_text}"
                )
        ...
        data = {
            ...
            "messages": [
                ...
                },
                {
                    "role": "user",
                    "content": (
                        f"Translate the following JSON values from English to {lang_english}. "
                        f"Do NOT modify any keys. Output raw JSON only, starting with {{ and ending with }}.\n\n"
                        f"{prompt_content}"
                    )
                }
            ],
