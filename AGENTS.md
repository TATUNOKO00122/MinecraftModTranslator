...
# Debug mode (with console visible)
run_debug.bat  # Windows: activates .venv, runs main.py, pauses on exit

# Build standalone EXE (PyInstaller)
pip install pyinstaller
pyinstaller MinecraftModTranslator.spec  # Uses spec file with correct data files

# Install dependencies
pip install PySide6 requests ftb_snbt_lib keyring
```
...
No lint/typecheck commands are configured. No `.vscode/settings.json` content.
...
### Crash Handling
- Global exception hook in `main.py`: `_global_excepthook()` catches unhandled exceptions, shows `QMessageBox.critical`
- Frozen exe logs: stdout/stderr redirected to `logs/error.log` when running as PyInstaller exe
- `faulthandler.enable()` for crash dumps
